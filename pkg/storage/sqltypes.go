...
func (t *DBTime) Scan(src any) error {
	...
	default:
		return fmt.Errorf("storage.DBTime: unsupported scan type %T", src)
	}
	return nil
}
...
func (t *DBTime) parse(s string) error {
	...
	return fmt.Errorf("storage.DBTime: cannot parse %q", s)
}
...
func (j JSONSlice[T]) Value() (driver.Value, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("storage.JSONSlice: %w", err)
	}
	return string(b), nil
}
...
func (j *JSONSlice[T]) Scan(src any) error {
	b, err := srcBytes(src)
	if err != nil {
		return fmt.Errorf("storage.JSONSlice: %w", err)
	}
	if b == nil {
		*j = []T{}
		return nil
...
func (j JSONMap) Value() (driver.Value, error) {
	...
	b, err := json.Marshal(j)
	if err != nil {
		return nil, fmt.Errorf("storage.JSONMap: %w", err)
	}
	return string(b), nil
}
...
func (j *JSONMap) Scan(src any) error {
	b, err := srcBytes(src)
	if err != nil {
		return fmt.Errorf("storage.JSONMap: %w", err)
	}
	if b == nil {
		*j = JSONMap{}
		return nil
